...
nest.ResetKernel()
#nest.SetKernelStatus({"resolution": 0.05, "print_time": False, "overwrite_files": True})

vinit=[ x for x in np.random.randn(400)*50.-50. ]
...
nest.CopyModel("static_synapse","inhibitory",{"weight":-300., "delay":3.0})

conlist = list(np.genfromtxt("../connections.ssv").astype(int))
for pre,cs in enumerate(conlist):
	for post in list(cs):
		nest.Connect([neurons[pre]], [neurons[post]], syn_spec="inhibitory")
